...
# 设置/EnigmaPrinter目录权限为777
RUN chmod 777 /EnigmaPrinter

ENV PYTHONPATH="/EnigmaPrinter"
...
# docker build -t enigmaprinter .

# 2. Run the Docker container using the following command
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/EnigmaPrinter/config.toml -v $(pwd)/storage:/EnigmaPrinter/storage -p 127.0.0.1:8501:8501 enigmaprinter
## For Windows:
# docker run -v ${PWD}/config.toml:/EnigmaPrinter/config.toml -v ${PWD}/storage:/EnigmaPrinter/storage -p 127.0.0.1:8501:8501 enigmaprinter
